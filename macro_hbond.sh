...
h_qscore_pdb_dir=/Users/davidboyer/Dropbox/Tau_Project_EISENBERG_LAB/amyloid_width/python/AB-Fibril-Radius-vs-Hydrogen-Bond-Relationship/pdb_h_qscore/
cpus=4
...
#Reset the cmd list file
echo "" > ${h_qscore_pdb_dir}/cmd_list.cmds

#Generate Run script commands (cmds) for h-bonds csvs
while read line  
do
  echo "python3 ${scripts_dir}/find_hbonds_04252021_debugged.py ${line} ${h_qscore_pdb_dir}" >> ${h_qscore_pdb_dir}/cmd_list.cmds;
  echo "" >> ${h_qscore_pdb_dir}/cmd_list.cmds;
done < ${h_qscore_pdb_dir}/pdb_files.txt

parallel --jobs ${cpus} < ${h_qscore_pdb_dir}/cmd_list.cmds &> ${h_qscore_pdb_dir}/run.macro_hbond.stdouterr
...
ls ${h_qscore_pdb_dir}/*datatable.csv > ${output_graphs_dir}/datatables_files.txt
ls ${h_qscore_pdb_dir}/*hbonds.csv > ${output_graphs_dir}/hbonds_files.txt
